...
function love.mousepressed(x, y, button, istouch)
    ...
    elseif keyboard['pressingn'] then
        map:cycle_angle(camera:x_screen_to_world(x), camera:y_screen_to_world(y))
    elseif keyboard['pressingb'] then
        map:cycle_fit(camera:x_screen_to_world(x), camera:y_screen_to_world(y))
    else
        ...
        if map.background_tiles[clicked_tile].contains_obstacle == false then
            pathfinding.fill_waypoints(gameobjects[i_player],
                                       map.background_tiles[clicked_tile].left +
                                           (map.background_tiles[clicked_tile]
                                               .width / 2),
...
function love.update(dt)
    ...
    if keyboard.pressingright and math.abs(gameobjects[i_player].x_velocity) <
        ...
        gameobjects[i_player].max_speed_while_rotating and
        gameobjects[i_player].colliding == false then
...
function love.draw()
    ...
    for i = 1, #map.background_tiles do
        ...
        if images[map.background_tiles[i].image] ~= nil then
            ...
            love.graphics.draw(images[map.background_tiles[i].image],
                               ...
                               (map.background_tiles[i].width / sprite_width) *
                                   camera.zoom,
                               (map.background_tiles[i].height / sprite_height) *
                                   camera.zoom, sprite_width / 2,
                               sprite_height / 2)
        end
    ...
    if i_grabbing ~= nil then
        ...
        love.graphics.rectangle("line", love.mouse.getX() -
                                    ((gameobjects[i_grabbing].width *
                                        camera.zoom) / 2), love.mouse.getY() -
                                    ((gameobjects[i_grabbing].height *
                                        camera.zoom) / 2),
