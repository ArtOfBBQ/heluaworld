        ...
        for j = 1, #gameobjects, 1 do
            if i == j then
                ...
                if gameobjects[j].angle == 0 or gameobjects[j].angle == 3.14 then
                    ...
                    if collision.are_unrotated_object_corners_colliding(i, j) then
                    ...
                    if collision.are_rotated_object_corners_colliding(i, j) then
                        collision.register_collision(i, j)
                    end
                end
            end
...
function collision.register_collision(i, j)
    ...
    if math.abs((gameobjects[j].x_velocity + 1) * gameobjects[j].weight) >
        math.abs((gameobjects[i].x_velocity + 1) * gameobjects[i].weight) then
        gameobjects[i].x = gameobjects[i].x +
                               ((gameobjects[i].x - gameobjects[j].x) * 0.005 *
                                   (2 + gameobjects[i].x_velocity))
        gameobjects[i].x_velocity = 0
    else
        gameobjects[j].x = gameobjects[j].x +
                               ((gameobjects[j].x - gameobjects[i].x) * 0.005 *
                                   (2 + gameobjects[j].x_velocity))
        gameobjects[j].x_velocity = 0
    end

    if math.abs((gameobjects[j].y_velocity + 1) * gameobjects[j].weight) >
        math.abs((gameobjects[i].y_velocity + 1) * gameobjects[i].weight) then
        gameobjects[i].y = gameobjects[i].y +
                               ((gameobjects[i].y - gameobjects[j].y) * 0.005 *
                                   (2 + gameobjects[i].y_velocity))
        gameobjects[i].x_velocity = 0
    else
        gameobjects[j].y = gameobjects[j].y +
                               ((gameobjects[j].y - gameobjects[i].y) * 0.005 *
                                   (2 + gameobjects[j].y_velocity))
