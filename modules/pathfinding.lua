...
function pathfinding.reverse_table_inplace(table_to_reverse)
    ...
    if table_to_reverse == nil then error("Expected table to reverse, got nil.") end
    if type(table_to_reverse) ~= "table" then error("Expected table to reverse, got " .. type(table_to_reverse) .. ".") end
    ...
    for i = 1, #map.background_tiles, 1 do
        ...
        for j = 1, #gameobjects, 1 do
            ...
            if gameobjects[j].max_speed == 0 then
                ...
                if collision.point_collides_unrotated_rectangle(
                    gameobjects[j].topleft_x,
                    gameobjects[j].topleft_y,
                    map.background_tiles[i].left,
                    map.background_tiles[i].top,
                    map.background_tiles[i].width,
                    map.background_tiles[i].height) then
                ...
                if collision.point_collides_unrotated_rectangle(
                    gameobjects[j].topright_x,
                    gameobjects[j].topright_y,
                    map.background_tiles[i].left,
                    map.background_tiles[i].top,
                    map.background_tiles[i].width,
                    map.background_tiles[i].height) then
                ...
                if collision.point_collides_unrotated_rectangle(
                    gameobjects[j].bottomleft_x,
                    gameobjects[j].bottomleft_y,
                    map.background_tiles[i].left,
                    map.background_tiles[i].top,
                    map.background_tiles[i].width,
                    map.background_tiles[i].height) then
                ...
                if collision.point_collides_unrotated_rectangle(
                    gameobjects[j].bottomright_x,
                    gameobjects[j].bottomright_y,
                    map.background_tiles[i].left,
                    map.background_tiles[i].top,
                    map.background_tiles[i].width,
                    map.background_tiles[i].height) then
...
function pathfinding.get_direct_neighbor_tile_indexes(i_tile, tiles_per_row, tiles_per_col)
    ...
    -- directly left
    if is_in_col_one == false then return_values[#return_values + 1] = i_tile - tiles_per_col end
...
function pathfinding.find_lowest_in_nodelist(nodelist, properties_to_compare)
    ...
    if type(properties_to_compare) ~= "table" then
        error("Expected a table of properties to compare, got " .. type(properties_to_compare))
    end

    -- find node with lowest fcost in nodelist
    lowest_node = nil
    for i = 1, #nodelist, 1 do
        if nodelist[i][properties_to_compare[1]] == nil then error("Property to compare: " .. properties_to_compare[1] .. " wasn't found in node " .. i .. "!") end
        ...
        elseif nodelist[i] ~= nil and nodelist[i][properties_to_compare[1]]
            ...
            if #properties_to_compare > 1 then
                ...
                if nodelist[i] ~= nil
                   and nodelist[i][properties_to_compare[1]] == nodelist[lowest_node][properties_to_compare[1]]
                   and nodelist[i][properties_to_compare[2]] < nodelist[lowest_node][properties_to_compare[2]] then
...
function pathfinding.single_astar_step(gameobject, target_x, target_y)
    ...
    i_cur_tile = pathfinding.find_lowest_in_nodelist( open_nodes, {"fcost", "gcost"} )
    pathfinding.move_index_from_table_to_table(open_nodes, closed_nodes, i_cur_tile)
    ...
    if closed_nodes[i_cur_tile].i_tile == i_target_tile then
            ...
            repeat
                gameobject.waypoints[#gameobject.waypoints + 1] = {
                    x = map.background_tiles[closed_nodes[i_cur_tile].i_tile].left + 25,
                    y = map.background_tiles[closed_nodes[i_cur_tile].i_tile].top + 25}
    ...
    for i = 1, #neighboring_nodes, 1 do
        ...
        neighboring_nodes[i] = { i_tile = neighboring_nodes[i], i_closed_nodes_parent = i_cur_tile }

        neighboring_nodes[i]["hcost"] = pathfinding.distance_between_points(
            map.background_tiles[neighboring_nodes[i].i_tile].left + (map.tile_width / 2),
            map.background_tiles[neighboring_nodes[i].i_tile].top + (map.tile_height / 2),
            gameobject.x,
            gameobject.y)
        ...
        neighboring_nodes[i]["gcost"] = pathfinding.distance_between_points(
            map.background_tiles[neighboring_nodes[i].i_tile].left + (map.tile_width / 2),
            map.background_tiles[neighboring_nodes[i].i_tile].top + (map.tile_height / 2),
            map.background_tiles[closed_nodes[i_cur_tile].i_tile].left + (map.tile_width / 2),
            map.background_tiles[closed_nodes[i_cur_tile].i_tile].top + (map.tile_height / 2))
            + closed_nodes[i_cur_tile].gcost
        
        neighboring_nodes[i]["fcost"] = neighboring_nodes[i].gcost + neighboring_nodes[i].hcost
    end
