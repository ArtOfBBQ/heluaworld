...
describe("We can calculate the distance between 2 points.", function()
    describe("Distance between points [2, 2] and [4, 4] should be about 2.82.",
        ...
        it(
            ...
            function()
                assert.is_true(pathfinding.distance_between_points(2, 2, 4, 4) >
                                   2.81)
            end)
        it(
    ...
    describe("Distance between points [2, 8] and [3, -2] should be about 10.05",
        ...
        it(
            ...
            function()
                assert.is_true(
                    pathfinding.distance_between_points(2, 8, 3, -2) > 10.04)
            end)
        it(
            "Therefore distance_between_points(2, 8, 3, -2) < 10.06 should be true.",
            function()
                assert.is_true(
                    pathfinding.distance_between_points(2, 8, 3, -2) < 10.06)
            end)
    end)
end)
...
describe("The neighboring tiles of a given map tile index can be found.",
    ...
    it("get_neighbor_tile_indexes() should always return a table.", function()
        assert.are_equal(type(pathfinding.get_neighbor_tile_indexes(1, 10, 10)),
                         "table")
    end)
    describe(
        ...
        function()
            it("We should have 3 indexes in our result.", function()
                assert.are_equal(#(pathfinding.get_neighbor_tile_indexes(1, 10,
                                                                         10)), 3)
            end)
            ...
            it(
                ...
                function()
                    local result = pathfinding.get_neighbor_tile_indexes(1, 10,
                                                                         10)
    ...
    describe(
        ...
        function()
            it("We should have 3 indexes in our result.", function()
                assert.are_equal(#(pathfinding.get_neighbor_tile_indexes(2, 10,
                                                                         10)), 5)
            end)
        ...
        describe("Throws with incorrect inputs.", function()
            ...
            it(
                ...
                function()
                    ...
                    end, "Expected a table of properties to compare, got string")
                end)
            it(
                ...
                function()
                    ...
                    assert.has_error(function()
                        pathfinding.find_lowest_in_nodelist(
                            {{fcost = 2}, {fcost = 1}}, 5)
                    end, "Expected a table of properties to compare, got number")
                end)
            it("Throws when it got a number, not a table of nodes", function()

                assert.has_error(function()
                    pathfinding.find_lowest_in_nodelist(5, {"fcost"})
                end, "Expected a table of nodes to compare, got number")
            end)
